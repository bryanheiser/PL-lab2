...
class Lab2Spec(lab2: Lab2Like) extends FlatSpec {
  ...
  "toNumber" should "convert to a number" in {
    ...
    assert(toNumber(e2) === 11.0)
    assert(toNumber(e3) === 4.0)
    assert(toNumber(e4).isNaN)
  }

  it should "throw exception when passed a string numbers and characters or just characters" in {
